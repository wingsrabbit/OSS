...
supply or replace accepted evidence.
...
commit-captured artifacts for state transitions, guard/RBAC, ledger postings,
transport, Provider contracts, data retention, threat modeling, and test
strategy. State, guard, and ledger counts are recomputed from schema-backed
documents, and every such document must have a passed independent review. A
pending or absent oracle therefore keeps G0 closed even when Feature Evidence
