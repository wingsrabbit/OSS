...
const schemaCompatibilityGuardName = SCHEMA_025_APPLICATION_GUARD;
...
try {
  if (config.OSS_SCHEMA_ROLLBACK_BRIDGE === "016-to-017") {
    throw new Error(
      "Schema 025 Worker refuses the legacy 016-to-017 rollback bridge; use the matching historical worker binary or migrate forward",
    );
  }
  ...
  try {
    ...
    await assertSchema025NativeSafe(queryable);
