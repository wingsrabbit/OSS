...
const schemaCompatibilityGuardName = SCHEMA_024_APPLICATION_GUARD;
...
try {
  if (config.OSS_SCHEMA_ROLLBACK_BRIDGE === "016-to-017") {
    throw new Error(
      "Schema 024 Worker refuses the legacy 016-to-017 rollback bridge; use the matching historical worker binary or migrate forward",
    );
  }
  ...
  try {
    ...
    await assertSchema024NativeSafe(queryable);
