...
export async function registerClientAccountRoutes(
  ...
  app.get("/api/v1/admin/client-accounts", async (request) => {
    ...
    const result = await pool.query<{
      ...
      `SELECT account.id,
       ...
       WHERE ($2::uuid IS NOT NULL AND account.id = $2::uuid)
       ...
       ORDER BY
         ...
         account.id
       LIMIT $3`,
      [query.query, uuidSearch.success ? uuidSearch.data : null, query.limit],
    );
    return {
      warning: LAB_WARNING,
  ...
  app.get(
    ...
    async (request) => {
      ...
      const params = z.object({ clientAccountId: canonicalUuid }).parse(request.params);
      const account = await loadAccountIdentity(pool, params.clientAccountId);
      return {
        warning: LAB_WARNING,
        account,
        items: await listOrders(pool, account.id),
      };
    },
  );
  ...
  app.get(
    ...
    async (request) => {
      ...
      const params = z.object({ clientAccountId: canonicalUuid }).parse(request.params);
      const account = await loadAccountIdentity(pool, params.clientAccountId);
      const invoices = await listInvoices(pool, account.id);
      const payments = await listPayments(pool, account.id);
      const credit = await loadCreditHistory(pool, account.id);
      const refunds = await listRefunds(pool, account.id);
      ...
      const debt = await pool.query<{ currency: string; balance_minor: string }>(
  ...
  app.get(
    ...
    async (request) => {
      ...
      return {
        warning: LAB_WARNING,
        account,
        items: await listServices(pool, account.id),
      };
    },
  );
  ...
  app.get(
    ...
    async (request) => {
      ...
      return {
        warning: LAB_WARNING,
        account,
        items: await listRenewals(pool, account.id),
      };
    },
  );
  ...
  app.get(
    ...
    async (request) => {
      ...
      return {
        warning: LAB_WARNING,
        account,
        items: await listCancellations(pool, account.id),
      };
    },
  );
  ...
  app.get(
    ...
    async (request) => {
      ...
      const params = z.object({ clientAccountId: canonicalUuid }).parse(request.params);
      const account = await loadAccountIdentity(pool, params.clientAccountId);
      const items = await listTickets(pool, account.id);
      const internalCounts = await pool.query<{ ticket_id: string; internal_count: string }>(
